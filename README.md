...
for d in dip_range:
    ...
    fault.readPatchesFromFile(faultdir+'.'.join(faultfile.split('.')[:-1])+'_dip'+str(d)+'.'+faultfile.split('.')[-1])   # personnal functions to import fault
...
uncert = uncer(faultdir, Np, multifault=False, export=True)
